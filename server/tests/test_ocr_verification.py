...
def test_ocr_verification_logic_with_synthetic_tokens():
    ...
    tokens = [
        {"text": "Hb", "clean_text": "hb", "left": 50, "top": 100, "width": 30, "height": 15, "conf": 95.0},
        {"text": "13.2", "clean_text": "13.2", "left": 120, "top": 102, "width": 35, "height": 14, "conf": 94.0},
        {"text": "g/dL", "clean_text": "g/dl", "left": 165, "top": 102, "width": 30, "height": 14, "conf": 92.0},
    ]
    ...
    # 3. Label not present -> NOT_FOUND
    res3 = ocr.verify_finding("Creatinine", "1.1", tokens)
    assert res3.verification_status == VerificationStatus.NOT_FOUND

    # 4. Low confidence token -> UNCLEAR
    low_conf_tokens = [
        {"text": "Hb", "clean_text": "hb", "left": 50, "top": 100, "width": 30, "height": 15, "conf": 95.0},
        {"text": "13.2", "clean_text": "13.2", "left": 120, "top": 102, "width": 35, "height": 14, "conf": 35.0},  # < 50
    ]
...
def test_ocr_search_multiline_fallback():
    ocr = OCRVerificationService()
    label_token = {"text": "Platelets", "clean_text": "platelets", "left": 50, "top": 100, "width": 60, "height": 20, "conf": 90.0}

    # Number is located on the wrapped row immediately below the label
    tokens = [
        label_token,
        {"text": "150,000", "clean_text": "150,000", "left": 55, "top": 130, "width": 50, "height": 18, "conf": 89.0},
    ]
    result = ocr.search_numeric_value_near_label(tokens, label_token)
