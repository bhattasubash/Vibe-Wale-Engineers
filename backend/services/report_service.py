...
class ReportService:
    ...
    async def process_all_reports(
        ...
        1. Generates a patient session ID
        2. Saves original images locally
        3. Groups images into reports
        4. Calls Gemini for individual structured report extractions
        5. Runs independent Tesseract OCR numerical verification against ORIGINAL images
        6. Persists each individual report JSON
        7. Calls Gemini to synthesize an overall longitudinal patient history
        8. Persists the overall patient summary JSON
        9. Returns response
        """
        if not files:
        ...
        # 1. Save uploaded images and get metadata
